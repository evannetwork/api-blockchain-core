...
Adds the traget account or identity to a specific role.

The main principle is that accounts or identities can be assigned to roles and those roles can be granted capabilities. :ref:`Function Permissions <rights-and-roles_setFunctionPermission>` are basically the capability to call specific functions if the calling account or identity belongs to a certain role. To add an account or identity to the role 'member'.
...
#. ``accountId`` - ``string``: executing accountId or identity
#. ``targetAccountId`` - ``string``: target accountId or identity
#. ``role`` - ``number``: roleId
...
The contract from this example has an owner (``0x0000000000000000000000000000000000000001``) and a member (``0x0000000000000000000000000000000000000002``). As the owner account or identity has the member role as well, it is listed among the members.
